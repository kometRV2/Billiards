...
int next_ball_number = 1;   //次に当てるべき球の番号
/*次のボールがどれかを判定する*/
void exist_check()
{
...
int collision_num = 0;      //球の衝突回数、間違った球に当たったかどうかの判定に使う
...
void count_Speed()
{
    ...
    if (balls_move_count > 0) {
        ...
        if (penalty || collision_num == 0) {//ペナルティー時はランダムな点に置かれる
        ...
        if (balls[0].exist == false) {
            Rabdom_Set(9,4,17,18);     //random_point[]にランダムな点を入れる
            balls[0].x = random_point[0];
            balls[0].z = random_point[1];
            balls[0].exist = true;
        }
        exist_check();      //次の球の番号を決める
...
{
    double shot_Speed = (2.0/15.0) * power + (2.0/3.0);  //打つ球の速さ、ゲージとリンクさせる
